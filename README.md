...
- **HXRV**（v1.0.1以降）: レビューのピンコメントが自動でログ化。コメントは対応指示にも反映され、対象要素のセレクタ・ページURLが付きます。HXRV v1.2.0以降の Before / After（現状・あるべき姿）も、詳細・対応指示に「Before（現状）→ After（あるべき姿）」ブロックとして取り込まれます
...
- **v1.4.1** — HXRV連携がBefore/After（HXRV v1.2.0）を取り込むよう対応
